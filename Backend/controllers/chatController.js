...
exports.chat = async (req, res) => {
  try {
    ...
    // Check if the query is about appointments
      ...
      const appointmentList = appointments
        .map((app) => {
          ...
          const apptTime = app.appointmentTime ? new Date(app.appointmentTime).toLocaleString() : "Unknown time";
    ...
    const ollamaEndpoint = process.env.OLLAMA_API_URL || "http://localhost:11434/v1/completions";
    ...
    // Extract the reply
    const reply =
      response.data.choices && response.data.choices[0] && response.data.choices[0].text
